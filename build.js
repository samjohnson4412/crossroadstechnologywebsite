...
`
  ...
  // Apache config for DreamHost and other traditional shared hosting, which
  // ignores _headers and _redirects entirely. Generated from the same site
  // config so the two stay in agreement.
  const host = site.origin.replace(/^https?:\/\//, '');
  const hostRe = host.replace(/\./g, '\\.');
  write(
    '.htaccess',
    `# Crossroads Technology — generated by build.js. Do not edit by hand.
...
<IfModule mod_rewrite.c>
  ...
  # Canonical host. Scoped to the live domain so a staging or preview
  # subdomain is left alone — delete nothing, just deploy elsewhere to test.
  RewriteCond %{HTTP_HOST} ^(www\\.)?${hostRe}$ [NC]
...
<IfModule mod_deflate.c>
  AddOutputFilterByType DEFLATE text/html text/css text/plain text/xml image/svg+xml application/json application/manifest+json
</IfModule>
